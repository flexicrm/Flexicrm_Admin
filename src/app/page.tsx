    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
        <Box display="flex" alignItems="center" sx={{ cursor: 'pointer' }}>
            ...
            <Typography variant="h6" component="span"
                sx={{ ml: 1, fontFamily: 'Sora, sans-serif', fontWeight: 800, color: '#2D3142', letterSpacing: '0.5px' }}>
                Flexi <span style={{ color: '#1E5FA8' }}>CRM</span>
            </Typography>
            <Chip label="BETA" size="small"
                sx={{ ml: 1.5, backgroundColor: '#F5921E', color: 'white', fontWeight: 'bold', fontSize: '0.65rem', height: 20 }} />
        </Box>
    </motion.div>
);
...
const AnimatedCounter: React.FC<{ value: string; color?: string }> = ({ value, color }) => {
    ...
    return <Typography variant="h4" sx={{ fontWeight: 800, color }}>{count}{suffix}</Typography>;
};
...
const SectionHeader = ({ label, title, sub }: { label: string; title: React.ReactNode; sub: string }) => (
    <ScrollReveal>
        <Box sx={{ textAlign: 'center', mb: 7 }}>
            ...
            <Typography variant="h2" sx={{ fontWeight: 800, color: '#2D3142', mb: 2, fontSize: { xs: '1.8rem', md: '2.4rem' } }}>{title}</Typography>
...
export default function Home(): React.JSX.Element {
    ...
    const drawer = (
        <Box sx={{ width: 260, pt: 2 }}>
            <Box sx={{ px: 2, mb: 2 }}><FlexiLogo /></Box>
            <List>
                {navItems.map((item) => (
            ...
            <Box sx={{ px: 2, mt: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                <Button variant="outlined" href="/register" fullWidth sx={{ borderColor: '#1E5FA8', color: '#1E5FA8', borderRadius: 2, fontWeight: 600 }}>Login</Button>
                <Button variant="contained" href="/register" fullWidth sx={{ bgcolor: '#F5921E', '&:hover': { bgcolor: '#d97b0d' }, borderRadius: 2, fontWeight: 700 }}>Register Now</Button>
            </Box>
        </Box>
    );
    ...
    return (
        <Box sx={{ overflowX: 'hidden', backgroundColor: '#ffffff', fontFamily: "'DM Sans', sans-serif", '& h1,& h2,& h3,& h4,& h5': { fontFamily: "'Sora', sans-serif" } }}>
            ...
            {/* ═══════ NAV ═══════ */}
            <AppBar position="fixed" sx={{ background: scrolled ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', boxShadow: scrolled ? '0 8px 32px rgba(30,95,168,0.10)' : 'none', borderBottom: '1px solid #E5E7EB', transition: 'all 0.35s ease' }}>
                <Toolbar sx={{ justifyContent: 'space-between', py: 1, height: 68 }}>
                    <FlexiLogo />
                    <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 4 }}>
                        {navItems.map((item) => (
                            <Button key={item} href={`#${item.toLowerCase().replace(/\s+/g, '')}`}
                                sx={{ color: '#2D3142', fontSize: '0.93rem', fontWeight: 500, textTransform: 'none', position: 'relative', '&::after': { content: '""', position: 'absolute', bottom: 2, left: '50%', transform: 'translateX(-50%)', width: 0, height: 2, bgcolor: '#1E5FA8', borderRadius: 1, transition: 'width 0.25s ease' }, '&:hover::after': { width: '70%' }, '&:hover': { color: '#1E5FA8', backgroundColor: 'transparent' } }}>
                                {item}
                            </Button>
                        ))}
                    </Box>
                    <Box gap={2} display="flex" alignItems="center">
                        <Button variant="text" href="/register" sx={{ display: { xs: 'none', sm: 'flex' }, color: '#1E5FA8', fontWeight: 600, border: '1.5px solid #1E5FA8', borderRadius: '8px', px: 2.5, py: 1, '&:hover': { backgroundColor: '#1E5FA8', color: '#fff' }, transition: 'all 0.25s ease' }}>Login</Button>
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <Button variant="contained" href="/register" sx={{ backgroundColor: '#F5921E', '&:hover': { backgroundColor: '#d97b0d' }, fontWeight: 700, px: 3, py: 1, borderRadius: '8px', boxShadow: '0 4px 14px rgba(245,146,30,0.35)', display: { xs: 'none', sm: 'flex' } }}>Register Now</Button>
                        </motion.div>
                        <IconButton sx={{ display: { xs: 'flex', md: 'none' } }} onClick={() => setMobileOpen(true)}><MenuIcon /></IconButton>
                    </Box>
                </Toolbar>
            </AppBar>
            <Drawer anchor="right" open={mobileOpen} onClose={() => setMobileOpen(false)}>{drawer}</Drawer>
            ...
            <Box ref={heroRef} id="home" sx={{ minHeight: '100vh', pt: { xs: 15, md: 20 }, pb: { xs: 10, md: 15 }, background: 'linear-gradient(135deg, #f8f9fb 0%, #eef3fb 60%, #fdf5ec 100%)', position: 'relative', overflow: 'hidden' }}>
                ...
                {[...Array(6)].map((_, i) => (
                    <motion.div key={i} style={{ position: 'absolute', width: 6 + i * 2, height: 6 + i * 2, borderRadius: '50%', background: i % 2 === 0 ? 'rgba(30,95,168,0.18)' : 'rgba(245,146,30,0.18)', top: `${10 + i * 12}%`, left: `${5 + i * 8}%` }}
                        animate={{ y: [0, -18, 0], opacity: [0.5, 1, 0.5] }} transition={{ duration: 3 + i, repeat: Infinity, delay: i * 0.4 }} />
                ))}
                <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 2 }}>
                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'center', gap: 6, minHeight: '70vh' }}>
                            ...
                            <motion.div variants={heroContainer} initial="hidden" animate="visible">
                                <motion.div variants={heroItem}>
                                    <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, mb: 2.5, px: 2, py: 0.75, borderRadius: 20, border: '1px solid rgba(34,197,94,0.3)', bgcolor: 'rgba(34,197,94,0.06)' }}>
                                        <Box component={motion.span} animate={{ scale: [1, 1.5, 1], opacity: [1, 0.5, 1] }} transition={{ duration: 1.5, repeat: Infinity }} sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: '#22c55e', display: 'block' }} />
                                ...
                                <motion.div variants={heroItem}>
                                    <Typography variant="h1" sx={{ fontWeight: 800, color: '#2D3142', mb: 2, fontSize: { xs: '2.4rem', md: '3.1rem' }, lineHeight: 1.12, letterSpacing: '-0.02em' }}>
                                        Manage Leads{' '}
                                        <motion.span style={{ color: '#1E5FA8', display: 'inline-block' }} animate={{ scale: [1, 1.02, 1] }} transition={{ duration: 3, repeat: Infinity }}>Smarter</motion.span>{' '}
                                        with Flexi CRM
                                    </Typography>
                                </motion.div>
                                <motion.div variants={heroItem}>
                                ...
                                <motion.div variants={heroItem}>
                                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mb: 5 }}>
                                        <motion.div whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                                            <Button variant="contained" size="large" href="/register" endIcon={<ArrowForwardIcon />}
                                        ...
                                        <motion.div whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                                            <Button variant="outlined" size="large" startIcon={<span style={{ fontSize: '1.1rem' }}>▶</span>}
                                                sx={{ borderColor: '#1E5FA8', color: '#1E5FA8', borderWidth: 2, px: 4, py: 1.75, fontSize: '1rem', fontWeight: 700, borderRadius: '10px', '&:hover': { borderWidth: 2, backgroundColor: alpha('#1E5FA8', 0.04) } }}>
                                                View Demo
                                            </Button>
                                        </motion.div>
                                    </Stack>
                                ...
                                <motion.div variants={heroItem}>
                                    <Stack direction="row" spacing={4} ref={statsRef}>
                                        {stats.map((stat, i) => (
                                            <Box key={i}>
                                                {statsVisible ? <AnimatedCounter value={stat.value} color={stat.color} /> : <Typography variant="h4" sx={{ fontWeight: 800, color: stat.color }}>0</Typography>}
                                                <Typography variant="body2" sx={{ color: '#6B7280', mt: 0.5 }}>{stat.label}</Typography>
                                            </Box>
                                        ))}
                                    </Stack>
                                </motion.div>
                        ...
                        <Box sx={{ flex: 1, position: 'relative', pt: { xs: 4, md: 0 }, pl: { md: 2 } }}>
                            <motion.div
                                initial={{ opacity: 0, y: -24, x: 10 }} animate={{ opacity: 1, y: 0, x: 0 }}
                            ...
                            <motion.div
                                initial={{ opacity: 0, x: 80, scale: 0.93 }} animate={{ opacity: 1, x: 0, scale: 1 }}
                                ...
                                <Grid container spacing={1} sx={{ mb: 2 }}>
                                    {[{ value: '248', label: 'Total Leads', color: '#1E5FA8' }, { value: '64', label: 'Follow-ups', color: '#F5921E' }, { value: '38', label: 'Converted', color: '#16a34a' }].map((m, idx) => (
                                    ...
                                    {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, idx) => {
                                        ...
                                        return (
                                            <Box key={day} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                                                <motion.div style={{ width: '100%', backgroundColor: colors[idx], borderRadius: '4px 4px 0 0' }}
                                                    initial={{ height: 0 }} animate={{ height: `${heights[idx]}%` }}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                    {[
                                        ...
                                        <motion.div key={idx} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }}
                                            transition={{ delay: 1.45 + idx * 0.13 }} whileHover={{ x: 4, transition: { duration: 0.2 } }}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1.5, backgroundColor: '#F3F4F6', borderRadius: 2, cursor: 'pointer', '&:hover': { backgroundColor: '#e8f0fb' }, transition: 'background 0.25s' }}>
            ...
            <Box sx={{ backgroundColor: '#F3F4F6', py: { xs: 8, md: 12 } }} id="whychooseus">
                <Container maxWidth="lg">
                    <SectionHeader
                        label="Why Choose Us"
                        title={<>Built for Teams That <span style={{ color: '#1E5FA8' }}>Move Fast</span></>}
                                ...
                                <RowSlide key={cardIndex} rowIndex={rowIndex} cardIndex={cardIndex}>
                                    <motion.div whileHover={{ y: -8, transition: { duration: 0.22 } }} style={{ height: '100%' }}>
                                        <Card sx={{
                                            ...
                                            <Typography sx={{ fontSize: '0.83rem', color: '#6B7280', lineHeight: 1.65 }}>
                                                {item.desc}
                                            </Typography>
                                        </Card>
                                    </motion.div>
            ...
            <Box sx={{ py: { xs: 8, md: 12 }, backgroundColor: '#fff' }} id="features">
                <Container maxWidth="lg">
                    <SectionHeader
                        label="Core Features"
                        title={<>Everything You Need to <span style={{ color: '#1E5FA8' }}>Manage Leads</span></>}
            ...
            <Box sx={{ background: 'linear-gradient(135deg, #0f2a4e 0%, #1a3f70 100%)', py: 12 }} id="roadmap">
                <Container maxWidth="lg">
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'relative', maxWidth: 1100, mx: 'auto' }}>
                        ...
                        <motion.div style={{ position: 'absolute', top: 28, left: 0, height: 2, backgroundColor: '#F5921E', zIndex: 1 }}
                            initial={{ width: '0%' }} whileInView={{ width: '10%' }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.5 }} />
                            ...
                            <ScrollReveal key={index} direction="up" delay={index * 0.18}>
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative', zIndex: 2, px: 1.5 }}>
                                    <motion.div whileHover={{ scale: 1.12 }} transition={{ duration: 0.2 }}
                                        style={{ width: 56, height: 56, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20, border: item.status === 'live' ? '3px solid rgba(245,146,30,0.4)' : '3px solid rgba(255,255,255,0.2)', backgroundColor: item.status === 'live' ? '#F5921E' : 'rgba(255,255,255,0.08)', boxShadow: item.status === 'live' ? '0 0 0 10px rgba(245,146,30,0.12)' : 'none' }}>
                                        {item.status === 'live' ? <motion.div animate={{ rotate: [0, 8, -8, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 1 }}>{item.icon}</motion.div> : item.icon}
                                    </motion.div>
                                    <Typography sx={{ fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: '0.96rem', color: '#fff', mb: 1.5 }}>{item.title}</Typography>
                                    <Chip label={item.status === 'live' ? '● Live Now' : 'Coming Soon'} size="small"
                                        sx={{ backgroundColor: item.status === 'live' ? '#22c55e20' : 'rgba(245,146,30,0.15)', color: item.status === 'live' ? '#4ade80' : '#F5921E', border: item.status === 'live' ? '1px solid #22c55e50' : '1px solid rgba(245,146,30,0.3)', fontWeight: 700, fontSize: '0.72rem' }} />
                                </Box>
                            </ScrollReveal>
                        ))}
                    </Box>
            ...
            <Box sx={{ backgroundColor: '#F3F4F6', py: 12 }} id="howitworks">
                <Container maxWidth="lg">
                    <SectionHeader
                        label="How It Works"
                        title={<>Up &amp; Running in <span style={{ color: '#1E5FA8' }}>4 Simple Steps</span></>}
                            ...
                            <ScrollReveal key={index} direction="down" delay={index * 0.18}>
                                <motion.div whileHover={{ y: -8, transition: { duration: 0.22 } }}>
                                    <Card sx={{ p: 4, height: '100%', borderRadius: 4, textAlign: 'center', border: '1px solid #E5E7EB', transition: 'all 0.3s', '&:hover': { boxShadow: '0 8px 32px rgba(30,95,168,0.12)' } }}>
                                        <motion.div whileHover={{ rotate: [0, -5, 5, 0], transition: { duration: 0.4 } }}>
                                            <Avatar sx={{ width: 60, height: 60, bgcolor: index === 3 ? '#F5921E' : '#1E5FA8', color: '#fff', fontSize: '1.2rem', fontWeight: 800, margin: '0 auto 20px', boxShadow: index === 3 ? '0 4px 16px rgba(245,146,30,0.35)' : '0 4px 16px rgba(30,95,168,0.3)' }}>{step.num}</Avatar>
                                        </motion.div>
                    ...
                    <Grid container spacing={8} alignItems="center">
                        <Grid item xs={12} md={6}>
                            <ScrollReveal direction="left">
                                ...
                                <Typography sx={{ fontSize: '1rem', color: 'rgba(255,255,255,0.65)', mb: 4, maxWidth: 460, lineHeight: 1.75 }}>
                                ...
                                <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
                                    {[{ Icon: PhoneIphoneIcon, line1: 'DOWNLOAD ON THE', line2: 'App Store' }, { Icon: AndroidIcon, line1: 'GET IT ON', line2: 'Google Play' }].map(({ Icon, line1, line2 }, idx) => (
                                        <motion.div key={idx} whileHover={{ scale: 1.05, y: -3 }}>
                                            <Button variant="outlined" startIcon={<Icon />} sx={{ backgroundColor: 'rgba(255,255,255,0.07)', border: '1.5px solid rgba(255,255,255,0.18)', borderRadius: 2, color: '#fff', py: 1.5, px: 3, '&:hover': { backgroundColor: 'rgba(255,255,255,0.13)', borderColor: '#F5921E' } }}>
                                ...
                                <Chip icon={<FiberManualRecordIcon sx={{ fontSize: 12, color: '#F5921E !important' }} />} label="App launching soon — Join the waitlist"
                        ...
                        <Grid item xs={12} md={6}>
                            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', gap: 3, height: 540, position: 'relative' }}>
                                ...
                                {/* Phone L */}
                                <motion.div initial={{ opacity: 0, x: -60, rotate: -6 }} whileInView={{ opacity: 1, x: 20, rotate: -6 }} viewport={{ once: true }} transition={{ duration: 0.9, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} whileHover={{ y: -10, rotate: -4, transition: { duration: 0.3 } }} style={{ width: 170, height: 360, borderRadius: 40, boxShadow: '0 40px 80px rgba(0,0,0,0.5),0 0 0 1px rgba(255,255,255,0.08)', overflow: 'hidden', position: 'relative', zIndex: 2, opacity: 0.85 }}>
                                    <Box sx={{ height: '100%', background: 'linear-gradient(180deg,#0d1a2e 0%,#0f2540 100%)' }}>
                                        ...
                                        <Box sx={{ position: 'absolute', inset: 0, padding: '40px 12px 12px', display: 'flex', flexDirection: 'column', gap: 1 }}>
                                            ...
                                            {[{ name: 'Ravi Shankar', tag: '🔥 Hot', tagBg: 'rgba(239,68,68,0.2)', tagColor: '#f87171' }, { name: 'Priya Nair', tag: 'Warm', tagBg: 'rgba(245,158,11,0.2)', tagColor: '#fbbf24' }, { name: 'Arjun Mehta', tag: 'New', tagBg: 'rgba(59,130,246,0.2)', tagColor: '#60a5fa' }].map((it, idx) => (
                                ...
                                {/* Phone C */}
                                <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }} whileHover={{ y: -10, transition: { duration: 0.3 } }} style={{ width: 200, height: 420, borderRadius: 40, boxShadow: '0 40px 80px rgba(0,0,0,0.5),0 0 0 1px rgba(255,255,255,0.08)', overflow: 'hidden', position: 'relative', zIndex: 3 }}>
                                    <Box sx={{ height: '100%', background: 'linear-gradient(180deg,#0d1a2e 0%,#0f2540 100%)' }}>
                                        <Box sx={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', width: 70, height: 20, background: '#000', borderRadius: 10, zIndex: 10 }} />
                                        <Box sx={{ position: 'absolute', inset: 0, padding: '40px 12px 12px', display: 'flex', flexDirection: 'column', gap: 1 }}>
                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                ...
                                                <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                                                    <Box component={motion.div} animate={{ scale: [1, 1.4, 1], opacity: [1, 0.5, 1] }} transition={{ duration: 1.5, repeat: Infinity }} sx={{ width: 7, height: 7, borderRadius: '50%', background: '#22c55e' }} />
                                                    <Typography sx={{ fontSize: '0.5rem', color: '#4ade80' }}>Live</Typography>
                                                </Box>
                                            </Box>
                                            <Grid container spacing={0.5}>
                                                {[{ val: '128', label: 'Total Leads', color: '#60a5fa' }, { val: '32', label: 'Follow-ups', color: '#F5921E' }, { val: '19', label: 'Converted', color: '#4ade80' }, { val: '74%', label: 'Response Rate', color: '#c084fc' }].map((m, i) => (
                                            ...
                                            <Box sx={{ background: 'rgba(255,255,255,0.04)', borderRadius: 1.5, p: 1, border: '1px solid rgba(255,255,255,0.06)' }}>
                                                ...
                                                <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 0.5, height: 40 }}>
                                                    {[40, 65, 50, 85, 70, 95, 45].map((h, idx) => (
                                                        <motion.div key={idx} style={{ flex: 1, background: idx === 4 || idx === 5 ? '#F5921E' : idx === 3 ? '#1E5FA8' : `rgba(30,95,168,${0.3 + idx * 0.1})`, borderRadius: '3px 3px 0 0' }}
                                                            initial={{ height: 0 }} whileInView={{ height: `${h}%` }} viewport={{ once: true }} transition={{ delay: 0.5 + idx * 0.08, duration: 0.5 }} />
                                                    ))}
                                                </Box>
                                            </Box>
                                            <Typography sx={{ fontSize: '0.52rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600, px: 0.5 }}>Recent Leads</Typography>
                                            {[{ name: 'Arun Kumar', company: 'TechCorp', initial: 'AK', color: '#1E5FA8', dot: '#22c55e' }, { name: 'Sneha Raj', company: 'Startup X', initial: 'SR', color: '#F5921E', dot: '#fbbf24' }, { name: 'Mohammed V', company: 'Global Traders', initial: 'MV', color: '#7c3aed', dot: '#f87171' }].map((lead, idx) => (
                                ...
                                <motion.div initial={{ opacity: 0, x: 60, rotate: 6 }} whileInView={{ opacity: 1, x: -20, rotate: 6 }} viewport={{ once: true }} transition={{ duration: 0.9, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} whileHover={{ y: -10, rotate: 4, transition: { duration: 0.3 } }} style={{ width: 170, height: 360, borderRadius: 40, boxShadow: '0 40px 80px rgba(0,0,0,0.5),0 0 0 1px rgba(255,255,255,0.08)', overflow: 'hidden', position: 'relative', zIndex: 2, opacity: 0.85 }}>
                                    <Box sx={{ height: '100%', background: 'linear-gradient(180deg,#0d1a2e 0%,#0f2540 100%)' }}>
                                        ...
                                        <Box sx={{ position: 'absolute', inset: 0, padding: '40px 12px 12px', display: 'flex', flexDirection: 'column', gap: 1 }}>
                                            ...
                                            {[{ title: '🔔 Follow-up Due', name: 'Ravi Shankar', sub: 'TechVision • 10:30 AM', color: '#F5921E', bg: 'rgba(245,146,30,0.1)', border: 'rgba(245,146,30,0.2)' }, { title: '✅ Lead Converted', name: 'Priya Nair', sub: 'Closed ₹2.4L deal', color: '#4ade80', bg: 'rgba(34,197,94,0.08)', border: 'rgba(34,197,94,0.2)' }, { title: '👤 New Lead Added', name: 'Karan Mehta', sub: 'Added by Sneha • 9:15 AM', color: '#60a5fa', bg: 'rgba(30,95,168,0.15)', border: 'rgba(30,95,168,0.25)' }].map((it, idx) => (
            ...
            <Box sx={{ background: 'linear-gradient(135deg, #1E5FA8 0%, #154080 100%)', py: 12, position: 'relative', overflow: 'hidden' }} id="cta">
                ...
                <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
                    <ScrollReveal>
                        <Typography variant="h2" sx={{ fontWeight: 800, color: '#fff', mb: 2, fontSize: { xs: '1.8rem', md: '2.8rem' } }}>Start Managing Your Leads Today</Typography>
                        <Typography sx={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.75)', mb: 4, maxWidth: 600, mx: 'auto' }}>
                        ...
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <Button variant="contained" size="large" href="/register"
                                sx={{ backgroundColor: '#F5921E', color: '#fff', px: 8, py: 2, fontSize: '1.1rem', fontWeight: 800, borderRadius: '10px', boxShadow: '0 6px 24px rgba(245,146,30,0.5)', '&:hover': { backgroundColor: '#d97b0d', boxShadow: '0 10px 32px rgba(245,146,30,0.6)' } }}>
                                Register Now — It&apos;s Free
                            </Button>
                        </motion.div>
                        <Typography sx={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.55)', mt: 2 }}>
                    ...
                    <Grid container spacing={6}>
                        <Grid item xs={12} md={4}>
                            <Box display="flex" alignItems="center" gap={1} sx={{ mb: 2 }}>
                                ...
                                <Typography sx={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: '1.2rem', color: '#fff' }}>Flexi <span style={{ color: '#4a90d9' }}>CRM</span></Typography>
                            </Box>
                        ...
                        {[
                            { title: 'Product', links: ['Features', 'Roadmap', 'Pricing', 'Changelog'] },
                            { title: 'Company', links: ['About Us', 'Blog', 'Careers', 'Press'] },
                            { title: 'Support', links: ['Help Center', 'Contact Us', 'Privacy Policy', 'Terms of Service'] },
                        ].map((col, idx) => (
                            <Grid item xs={12} sm={4} md={2} key={idx}>
                                <Typography sx={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: '0.9rem', color: '#fff', mb: 2 }}>{col.title}</Typography>
                                {col.links.map((link, li) => (
                                    <Button key={li} href="#" sx={{ display: 'block', color: 'rgba(255,255,255,0.55)', fontSize: '0.85rem', textAlign: 'left', py: 0.5, textTransform: 'none', '&:hover': { color: '#F5921E', backgroundColor: 'transparent' }, transition: 'color 0.2s' }}>{link}</Button>
                                ))}
                            </Grid>
                        ))}
                    </Grid>
                        ...
                        <Stack direction="row" spacing={1}>
                            {[TwitterIcon, LinkedInIcon, InstagramIcon].map((Icon, idx) => (
                                <motion.div key={idx} whileHover={{ scale: 1.15, y: -2 }}>
                                    <IconButton size="small" sx={{ width: 36, height: 36, backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 1, color: 'rgba(255,255,255,0.7)', '&:hover': { backgroundColor: '#F5921E', color: '#fff' }, transition: 'all 0.25s' }}>
                                        <Icon sx={{ fontSize: 18 }} />
                                    </IconButton>
                                </motion.div>
                            ))}
