...
export default function Page() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/activity/${subdomain}`,
          { headers }
        );
        setData(response.data.data.Activities);
      } catch (error) {
  ...
  const openDialog = async (activityId) => {
    try {
      const headers = { Authorization: `Bearer ${accessToken}` };
      const response = await axios.get(
        `${API_BASE_URL}/activity/${subdomain}/${activityId}`,
        { headers }
      );
  ...
  return (
    <div style={{ maxWidth: "1500px", margin: "0 auto", overflow: "auto" }}>
      ...
      >
        ...
        <Column field="userId.userRole" header="User Role" />
        <Column
          body={(rowData) => formatDate(rowData.entityId.firstLoginTime)}
          header="First Login Time"
        />
        <Column field="entityId.mobile" header="Mobile" />
        <Column
          body={(rowData) => (
            <button onClick={() => openDialog(rowData.entityId._id)}>
              View Details
            </button> // Use entityId here
          )}
          header="Details"
        />
      </DataTable>
              ...
              <div className="grid-item">
                ...
                <div className="text-center mt-2">
                  ...
                  <ul className="list-inline">
                    ...
                    <li><span className="fw-bold">actionType :</span> <br/>
                    {selectedActivity.actionType}</li>
                    <li><span className="fw-bold">description :</span>  <br/>
                      { selectedActivity.description}</li>
                  </ul>
                </div>
              </div>
            </Col>
