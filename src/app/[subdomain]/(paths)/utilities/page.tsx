...
// import { Box, Button, FormControl, InputLabel, MenuItem, Select, TextField, Typography, Paper, Snackbar, Alert, IconButton, Tooltip, useTheme, Grid } from '@mui/material';
...
const InputField: React.FC<{
    ...
    onChange: (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
    dragHandleProps: any;
}> = ({ label, type, name, value, onChange, dragHandleProps }) => (
    <Box
        mb={1}
        sx={{
            display: 'flex',
...
const DynamicForm: React.FC = () => {
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        if (source.droppableId === 'available' && destination.droppableId === 'available') {
            ...
            // Reorder selected fields in the form preview
            const reordered = [...selectedFields];
            const [removed] = reordered.splice(source.index, 1);
            reordered.splice(destination.index, 0, removed);
            setSelectedFields(reordered);
        } else {
            // Move between available and selected
            const sourceList = source.droppableId === 'available' ? [...availableFields] : [...selectedFields];
            const destList = destination.droppableId === 'available' ? [...availableFields] : [...selectedFields];
            const [removed] = sourceList.splice(source.index, 1);
            destList.splice(destination.index, 0, removed);
                                            ...
                                            <Draggable key={field.id} draggableId={field.id} index={index}>
                                                {(provided) => (
                                                    <div ref={provided.innerRef} {...provided.draggableProps}>
                                                        <InputField key={field.id} label={field.label} type={field.type} name={field.name} value={formData[field.name] || ''} onChange={handleInputChange} dragHandleProps={provided.dragHandleProps} />
                                                    </div>
                                                )}
                                            </Draggable>
                                        ))}
