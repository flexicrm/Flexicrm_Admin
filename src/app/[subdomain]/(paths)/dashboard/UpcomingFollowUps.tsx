...
import { Card, Box, Typography, Badge, List, ListItem, ListItemText, Divider, Skeleton } from '@mui/material';
  ...
  return (
    <Card sx={{ height: 350, borderRadius: 2, border: '1px solid #e5e7eb', boxShadow: '0 6px 30px rgba(182, 186, 203, 0.3)', padding: '16px', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        <Badge badgeContent={data?.length || 0} color="primary" max={99} />
      </Box>
      {isLoading ? (
        <>
          {[...Array(3)].map((_, i) => (
